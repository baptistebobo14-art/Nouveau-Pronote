...
darkModeStyle.textContent = `
    ...
    /* Zones avec un fond blanc écrit directement dans le HTML */
    ...
    /* Textes gris devenus presque invisibles */
    [style*="color:#666"],
    [style*="color:#777"],
    [style*="color:#888"],
    [style*="color:#aaa"] {
    ...
    /* Moyennes : éviter les gros espaces/zones blanches */
    .dashboard-card,
    .activity-card,
    .cards > div,
    .stat-card {
    ...
    /* 📄 BULLETIN */
    #bulletinContent,
    #bulletinContent table,
    #bulletinContent thead,
    #bulletinContent tbody,
    #bulletinContent tr,
    #bulletinContent th,
    #bulletinContent td {
        background:#1f2937 !important;
    ...
    /* Supprime les grands espaces/fonds blancs du bulletin */
    #bulletinContent > div {
        background:#1f2937 !important;
    }
    ...
    /* Tout élément blanc directement défini par un style inline */
    [style*="background:white"],
    [style*="background: white"],
    [style*="background:#fff"],
    [style*="background: #fff"],
    ...
    /* =====================================================
       🌙 CORRECTION DÉFINITIVE NOTES + CAHIER DE TEXTES
       ===================================================== */
    ...
    /* 📚 CAHIER DE TEXTES : colonnes des jours */
    #content [style*="background:#f8f9fb"],
    #content [style*="background: #f8f9fb"] {
        background:#1f2937 !important;
        color:#f3f4f6 !important;
        border-color:#374151 !important;
    ...
    /* 📅 EN-TÊTES : jour + date + mois */
    #content [style*="background:#f1f3f6"],
    #content [style*="background: #f1f3f6"] {
        background:#172033 !important;
        color:#f3f4f6 !important;
        border-color:#374151 !important;
    ...
    /* Tous les textes à l'intérieur des en-têtes */
    #content [style*="background:#f1f3f6"] *,
    #content [style*="background: #f1f3f6"] * {
        color:#f3f4f6 !important;
    }

    /* 📖 ZONE QUI CONTIENT LES COURS */
    #content [style*="background:#f8f9fb"] > div,
    #content [style*="background: #f8f9fb"] > div {
        color:#f3f4f6 !important;
    }

    /* 📝 COURS AVEC FOND BLANC */
    ...
    [class*="note-detail"] select {
        background:#111827 !important;
        color:#f3f4f6 !important;
        border-color:#4b5563 !important;
    }

    /* 🌙 DÉTAIL DE NOTE — MODE SOMBRE DÉFINITIF */

    #content .note-detail-page {
        background:#111827 !important;
        color:#f3f4f6 !important;
    }

    #content .note-detail-page .note-detail-header,
    #content .note-detail-page .note-detail-card,
    #content .note-detail-page .note-detail-info-card,
    #content .note-detail-page .note-detail-section,
    #content .note-detail-page .detail-info-item,
    #content .note-detail-page .note-statistics,
    #content .note-detail-page .note-history,
    #content .note-detail-page .note-information {
        background:#1f2937 !important;
        color:#f3f4f6 !important;
        border-color:#374151 !important;
    }

    #content .note-detail-page h1,
    #content .note-detail-page h2,
    #content .note-detail-page h3,
    #content .note-detail-page p,
    #content .note-detail-page span,
    #content .note-detail-page strong,
    #content .note-detail-page label {
        color:#f3f4f6 !important;
    }

    #content .note-detail-page .blue-text {
        color:#60a5fa !important;
    }

    #content .note-detail-page .back-btn,
    #content .note-detail-page .print-btn {
        color:white !important;
    }
...
async function changeMyPassword(){
    ...
    alert(
        ...
        "L'ancien mot de passe ne peut plus être utilisé."
    );
}
...
let edtSelectedWeek =
    localStorage.getItem("edtSelectedWeek") || "";
...
function emploiPage(){
    ...
    /*
     * 🔄 AUTOMATIQUE :
     * si la semaine enregistrée est passée,
     * on revient à la semaine actuelle.
     */

    if(
        !edtSelectedWeek ||
        edtSelectedWeek < currentWeek
    ){
    ...
    const weekDates =
        getCahierWeekDates(edtSelectedWeek);
        ...
        ">
            ...
            ">
                ...
                <input
                    type="date"
                    id="edtWeek"
                    value="${edtSelectedWeek}"
                    onchange="selectEdtWeek(this.value)"
                    style="
                        width:100%;
                        box-sizing:border-box;
                    "
...
function getCurrentWeekString(){
    ...
    const monday = getMonday(todayString);
...
function selectEdtWeek(value){

    if(!value) return;

    edtSelectedWeek = value;
...
function changeEdtWeek(offset){

    const input =
        document.getElementById("edtWeek");

    if(!input || !input.value) return;

    const monday =
        getMonday(input.value);
...
function buildPronoteGrid(weekDates){
...
function renderCourses(){
    ...
    const currentWeek =
        edtSelectedWeek || getCurrentWeekString();

    const coursesByDay = {};

    // ==========================================
    // 1. RÉCUPÉRER LES COURS DE LA BONNE SEMAINE
    // ==========================================
    ...
    DB.courses
        .filter(course => {
            ...
            if(!course.week){
                return currentWeek === getCurrentWeekString();
            }
    ...
    // ==========================================
    // 2. TRAITER CHAQUE JOUR SÉPARÉMENT
    // ==========================================
    ...
    Object.keys(coursesByDay).forEach(day => {
        ...
        // Ordre stable, même après refresh Supabase
        courses.sort((a,b) => {
        ...
        // ==========================================
        // 3. CRÉER LES GROUPES DE CHEVAUCHEMENT
        // ==========================================
        ...
        // ==========================================
        // 4. CALCULER LES COLONNES DE CHAQUE GROUPE
        // ==========================================
...
group.forEach(course => {
    course._laneCount = laneCount;
});

// ==========================================
// 🎠 CARROUSEL DES COURS QUI SE CHEVAUCHENT
// ==========================================
...
if(edtCarouselPositions[groupKey] === undefined){
    edtCarouselPositions[groupKey] = 0;
}

if(edtCarouselPositions[groupKey] >= group.length){
    edtCarouselPositions[groupKey] = 0;
}

const carouselIndex =
    edtCarouselPositions[groupKey];
...
// 🎠 Dans un groupe avec plusieurs cours,
// on affiche uniquement le cours sélectionné
if(group.length > 1 && course.id !== carouselCourse.id){
    return;
}
                ...
                const lane =
    Number(course._lane || 0);

const isCarousel =
    group.length > 1;

const displayWidth =
    isCarousel ? 100 : width;

const left =
    isCarousel ? 0 : lane * width;
    ...
    "
                        ...
                        style="
                            ...
                            grid-column:${Number(course.day) + 2};

                            grid-row:${start + 2}/${end + 2};

                            width:calc(${displayWidth}% - 6px);

margin-left:calc(${left}% + 3px);
                        "

                        onclick="openCourseActions(event, ${course.id})"
                    >

${
    group.length > 1
        ? `
            <div class="edt-carousel-nav">

                <button
                    type="button"
                    onclick="event.stopPropagation(); changeEdtCarousel('${groupKey}', -1)"
                >
                    ◀
                </button>

                <span>
                    ${carouselIndex + 1} / ${group.length}
                </span>
    ...
    ? `Cours ⚠️ déplacé 💱
        ...
        <span class="moved-from">
            ↪️ Initialement :
            ${EDT_DAYS[Number(course.originalDay)] || ""}
            ${EDT_HOURS[Number(course.originalStart)]?.split("-")[0] || ""}
            →
            ${EDT_HOURS[Number(course.originalEnd) - 1]?.split("-")[1] || ""}
        </span>`
...
function changeEdtCarousel(groupKey, direction){
    ...
    // Le nombre de cours correspond aux IDs après le jour
    const total =
    ...
    edtCarouselPositions[groupKey] = next;

    render();
}

function openCourseActions(event, courseId){
    ...
    const dayName =
        EDT_DAYS[Number(course.day)] || "";

    const dateText =
    course.week
        ? getCahierWeekDates(course.week)[Number(course.day)]
    ...
    overlay.innerHTML = `
        ...
        <div class="course-detail-window">
            ...
            <div class="course-detail-header">
                ...
                <button
                    class="course-detail-close"
                    onclick="closeCourseActions()"
                >
                    ✕
                </button>
            ...
            <div class="course-detail-body">
                ...
                <div class="course-detail-item">
                    ...
                    <div>
                        ...
                        <span>
                            ${EDT_HOURS[Number(course.start)]?.split("-")[0] || ""}
                            →
                            ${EDT_HOURS[Number(course.end)-1]?.split("-")[1] || ""}
                        </span>
                    </div>
                </div>
                ...
                <div class="course-detail-item">
                    ...
                    <div>
                        ...
                        <span>${course.teacher || "Non renseigné"}</span>
                    </div>
                </div>
                ...
                <div class="course-detail-item">
                    ...
                    <div>
                        <strong>Salle</strong>
                        <span>${course.room || "Non renseignée"}</span>
                    </div>
                </div>
                ...
                <div class="course-detail-item">
                    ...
                    <div>
                        ...
                        <span>${course.className || "Non renseignée"}</span>
                    </div>
                </div>

<div class="course-detail-item course-information-item">
    📝
    <div>
        <strong>Informations / contenu du cours</strong>
        <span>
            ${course.information || "Aucune information renseignée."}
        </span>
    </div>
</div>
            ...
            ${
                ...
                <div class="course-detail-actions">
                    ...
                    <div class="course-actions-title">
                        ⚙️ Actions du professeur
                    </div>

                    <button onclick="cancelCourse(${courseId})">
                        Annuler ❌ le cours
                    </button>

                    <button onclick="modifyCourse(${courseId})">
                        Modifier ✏️ le cours
                    </button>

                    <button onclick="teacherAbsent(${courseId})">
                        Prof 👨🏻‍🏫 absent
                    </button>

                    <button onclick="moveCourse(${courseId})">
                        Déplacer 💱 le cours
                    </button>
...
function closeCourseActions(){
    ...
    const panel =
        document.getElementById("courseActionsPanel");
    ...
    const overlay =
        document.getElementById("courseDetailOverlay");
...
document.addEventListener("click", function(event){
    ...
    if(
        ...
        !event.target.closest(".course-block-v2")
    ){
...
async function cancelCourse(courseId){
    ...
    if(error){
        ...
        console.error(
            "Erreur annulation cours :",
            error
        );

        alert("❌ Impossible d'annuler le cours.");
    ...
    const wantsMove = confirm(
        "❌ Le cours a bien été annulé.\n\n" +
        "🔄 Souhaitez-vous reprogrammer ce cours " +
        "à un autre créneau ?"
    );

    if(wantsMove){

        openRescheduleCourse(courseId);

    }

}

function openRescheduleCourse(courseId){
    ...
    const old =
        document.getElementById("rescheduleCourseOverlay");
    ...
    overlay.id = "rescheduleCourseOverlay";
    ...
    overlay.innerHTML = `
        ...
        <div class="course-detail-window">
            ...
            <div style="padding:20px;">
                ...
                <select id="rescheduleDay">

                    ${EDT_DAYS.map((day,index)=>`

                        <option
                            value="${index}"
                            ${Number(course.day) === index ? "selected" : ""}
                        >
                            ${day}
                        </option>
                ...
                <select id="rescheduleStart">

                    ${EDT_HOURS.map((hour,index)=>`
                ...
                <button
                    style="width:100%;"
                    onclick="saveRescheduledCourse(${courseId})"
                >
                    🔄 Reprogrammer le cours
                </button>
...
function closeRescheduleCourse(){
...
async function saveRescheduledCourse(originalCourseId){
    ...
    const newDay =
        Number(
            document.getElementById("rescheduleDay").value
        );
    ...
    const conflicts =
        DB.courses.filter(course => {
            ...
            if(course.week !== original.week){
                return false;
            }

            if(Number(course.day) !== newDay){
                return false;
            }
            ...
            return (
                Number(course.start) < newEnd &&
                Number(course.end) > newStart
            );
    ...
    if(conflicts.length > 0){
        ...
        const continueAnyway = confirm(
            ...
            "⚠️ Il y a déjà " +
            conflicts.length +
            " cours sur ce créneau.\n\n" +

            "Voulez-vous quand même reprogrammer le cours ici ?"
    ...
    const { data, error } = await supabaseClient
        ...
        .insert({
            ...
            original_day: Number(original.day),

            original_start_slot: Number(original.start),

            original_end_slot: Number(original.end)
    ...
    if(error){
        ...
        console.error(
            "Erreur reprogrammation cours :",
            error
        );

        alert(
            "❌ Impossible de reprogrammer le cours."
        );
    ...
    // ==========================================
    // ➕ AJOUTER LE NOUVEAU COURS À L'ÉCRAN
    // ==========================================
    ...
    DB.courses.push({
        ...
        start: Number(data.start_slot),

        end: Number(data.end_slot),
        ...
        originalCourseId:
            data.original_course_id,

        originalDay:
            data.original_day,

        originalStart:
            data.original_start_slot,

        originalEnd:
            data.original_end_slot

    });


    closeRescheduleCourse();

    render();


    alert(
        "✅ Le cours a été reprogrammé !"
    );
...
function modifyCourse(courseId){
    ...
    setTimeout(() => {
        ...
        document.getElementById("courseInformation").value =
    course.information || "";
...
async function teacherAbsent(courseId){
...
async function moveCourse(courseId){
...
function editCourse(id){
    ...
    course.room =
        prompt("Nouvelle salle :", course.room) || course.room;

    course.teacher =
        prompt("Professeur :", course.teacher) || course.teacher;
...
<label>📝 Informations sur le cours</label>

<textarea
    id="courseInformation"
    rows="4"
    placeholder="Ex : Correction du contrôle, début du chapitre 4, apporter le manuel..."
></textarea>
...
async function createCourse(){
    ...
    const information =
    document.getElementById("courseInformation").value.trim();

    const day =
        Number(document.getElementById("courseDay").value);

    const start =
        Number(document.getElementById("courseStart").value);

    const end =
        Number(document.getElementById("courseEnd").value);
    ...
    if(editingCourseId !== null){
        ...
        if(course){
            ...
            course.className = data.class_name || "";
            course.information = data.information || "";
            course.day = Number(data.day);
            course.start = Number(data.start_slot);
            course.end = Number(data.end_slot);
...
async function loadCourses(){
    ...
    DB.courses = (data || []).map(course => ({
        ...
        start: Number(course.start_slot),

        end: Number(course.end_slot),

        week: course.week,

        status: course.status || null,

originalCourseId: course.original_course_id ?? null,

originalDay: course.original_day ?? null,

originalStart: course.original_start_slot ?? null,

originalEnd: course.original_end_slot ?? null
...
function showNoteDetail(id){
    ...
    let lowest = "—";
    let median = "—";
    let differenceWithAverage = null;
    let comparisonText = "";
...
if(!isNaN(value) && !isNaN(outof) && outof > 0){
    ...
    const differenceFormatted =
        Math.abs(differenceWithAverage)
            .toFixed(2)
            .replace(".00","");

    if(differenceWithAverage > 0){

        comparisonText =
            `🟢 Ta note est ${differenceFormatted} point${differenceFormatted == "1" ? "" : "s"} au-dessus de la moyenne de la classe.`;

    }else if(differenceWithAverage < 0){

        comparisonText =
            `🔴 Ta note est ${differenceFormatted} point${differenceFormatted == "1" ? "" : "s"} en dessous de la moyenne de la classe.`;

    }else{

        comparisonText =
            `🟡 Ta note correspond exactement à la moyenne de la classe.`;
...
<div class="note-detail-section">
    ...
    ">
        ...
        ">
            ...
            ${comparisonText || "Les statistiques de comparaison ne sont pas disponibles."}
    ...
    <div class="note-statistics">
        ...
        <div>
            ...
            <small style="
                ...
                line-height:1.4;
            ">
                C'est la moyenne de toutes les notes
                obtenues à cette évaluation.
            </small>
        ...
        <div>
            ...
            <small style="
                ...
                line-height:1.4;
            ">
                La moitié des notes est en dessous
                et l'autre moitié au-dessus.
            </small>
        ...
        <div>
            ...
            <small style="
                ...
                line-height:1.4;
            ">
                C'est la note la plus élevée
                obtenue à cette évaluation.
            </small>
    ...
    ">
        ...
        La <strong>moyenne</strong> donne le niveau moyen
        de la classe.

        La <strong>médiane</strong> permet de voir
        comment les notes sont réparties autour du milieu.
