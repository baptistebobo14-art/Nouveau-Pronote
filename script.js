    ...
    Object.keys(coursesByDay).forEach(day=>{
        ...
        courses.forEach(course=>{
            ...
            while(
                ...
                lanes[lane] > course.start
            ){
                lane++;
            }

            course._lane=lane;
            lanes[lane]=course.end;
        ...
        courses.forEach(course=>{
            ...
            const width=100/Math.max(1,overlapping);

            const left=course._lane*width;

            html+=`

            <div
                class="course-block-v2 ${course.status === "cancelled" || course.status === "teacher_absent" ? "course-cancelled" : ""}"

                style="
                    background:${SUBJECT_COLORS[course.subject]||"#2563eb"};
                    grid-column:${Number(course.day)+2};
                    grid-row:${course.start + 2}/${course.end + 2};
